...
function App() {
        ...
        await ethereum.request({
          ...
          params: {
            ...
            options: {
              ...
              symbol: "VAI",
              decimals: 18,
              image: "https://svgshare.com/i/q03.svg",
            },
          },
        });
      } catch (error) {
    ...
    if (provider != undefined) {
      ...
      if (chainId != "0x13881") {
        ...
        if (maticInput == "normal") {
          ...
          valueCheck
            ? await contract.openLotterySplit(
                authorWallet,
                numOfWinners,
                rewardsNormalBigNumber,
                { value: ethers.utils.parseEther(maticValue.toString()) }
              )
            : alert("Rewards dont match");
        } else if (maticInput == "percentage") {
          await contract.openLotteryPercentage(
            authorWallet,
            numOfWinners,
            rewardsPercentage,
            { value: ethers.utils.parseEther(maticValue.toString()) }
    ...
    if (provider == undefined) {
      ...
      if (chainId !== "0x5") {
        ...
        await contract.transfer(
          "0xD2509e56a44B60D72eb4904AA24925043c0Eaf18",
          ethers.utils.parseEther(maticValue.toString())
        );
  ...
  const getNftAbi = (contractAddress) => {
    axios
      .get(
        `https://api-goerli.etherscan.io/api?module=contract&action=getabi&address=${contractAddress}&apikey=${process.env.REACT_APP_ETHERSCAN_API_KEY}`
      )
    ...
    if (provider == undefined) {
      ...
      if (chainId !== "0x5") {
        ...
        await contract.transferFrom(
          authorWallet,
          "0x438BA8BD834b6053B82C269c2D30d566FFC32baE",
          1
        );
        await handleOpenLottery();
      }
  ...
  return (
    <div>
      <div className="connect-button-container">
        <button onClick={() => switchChain("polygon")} className="">
          Switch to Mumbai
        </button>
        <button onClick={() => switchChain("goerli")} className="">
          Switch to Goerli
        </button>
        {wallet == "" && (
      ...
      <div className="main-container">
        <div className="title">What are you giving away?</div>
        <form className="title-form">
          <input
            type="checkbox"
            checked={checkedType === "MATIC"}
            id="matic"
            name="matic"
            onChange={() => setChecked("MATIC")}
          />
          <label for="matic">MATIC</label>
          <input
            type="checkbox"
            checked={checkedType === "ERC20"}
            id="ERC20"
            name="ERC20"
            onChange={() => setChecked("ERC20")}
          />
          <label for="ERC20">ERC20 Token</label>
          <input
            type="checkbox"
            checked={checkedType === "NFT"}
            id="NFT"
            name="NFT"
        ...
        {checkedType === "MATIC" && (
          <div>
            <form className="container">
              <div className="rewards-type">
                <input
                  ...
                  id="percentageInput"
                  checked={maticInput === "normal"}
                  onChange={() => setMaticCheck("normal")}
                />
                ...
                <input
                  ...
                  name="normalInput"
                  id="normalInput"
                  checked={maticInput === "percentage"}
                  onChange={() => setMaticCheck("percentage")}
                />
              ...
              {maticInput === "normal" && (
                <label>
                  ...
                  <input
                    className="input"
                    onChange={(e) => handleRewardsNormal(e.target.value)}
                  ></input>
                </label>
              )}
              {maticInput === "percentage" && (
                <label>
                  Percentage rewards for each winner:
                  <input
                    className="input"
                    onChange={(e) => handleRewardsPercentage(e.target.value)}
                  ></input>
                </label>
              )}
              <label>
            ...
            <input
              ...
              name="participants"
              className="input"
              onChange={(e) => handleSetParticipants(e.target.value)}
            />
            <button
            ...
            <br></br>
            <br></br>
            <button onClick={() => handleRequestRandomNumber()}>
              Pick random number and finish lottery
            </button>
            <br></br>
            <br></br>
            <div>
          ...
          <div>
            <form className="container">
              <div className="erc20-form">
                <input
                  type="checkbox"
                  checked={erc20Input === "ETH"}
                  id="ETH"
                  name="ETH"
                  onChange={() => setERC20("ETH")}
                />
                <label for="NFT">ETH</label>
                <input
                  type="checkbox"
                  checked={erc20Input === "USDT"}
                  id="USDT"
                  name="USDT"
                  onChange={() => setERC20("USDT")}
                />
                <label for="NFT">USDT</label>
                <input
                  type="checkbox"
                  checked={erc20Input === "USDC"}
                  id="USDC"
                  name="USDC"
                  onChange={() => setERC20("USDC")}
                />
        ...
        {checkedType === "NFT" && (
          <div>
            <form className="container">
              ...
              <label>
                ...
                <input
                  type="number"
                  className="input"
                  onChange={(e) => handleNumOfWinners(e.target.value)}
                ></input>
              </label>
              <label>
                How long should the lottery last? (in hours)
                  ...
                  <div className="winner-container">
                    ...
                    <div className="winner-info">
                      ...
                      <input
                        ...
                        id="nftCollection"
                        name="nftCollection"
                        onChange={(e) => updateNftAddresses(e.target.value, i)}
                      />
            ...
            <button
              onClick={() =>
                handleNftSubmit("0x48d9dE329dCb49533F1C9013cC3aABc5E82a0c28")
              }
            >
              Submit
            </button>
            <button onClick={() => addToken()}> CLICK ME</button>
          </div>
        )}
      </div>
    </div>
